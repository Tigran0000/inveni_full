# ui/pages/restore_page.py - Enhanced with responsive design (Timeline Removed)
...
from utils.time_utils import format_timestamp_dual
...
class RestorePage:
    ...
    def __init__(self, parent, version_manager, backup_manager, settings_manager, shared_state, colors=None, ui_scale=1.0, font_scale=1.0):
        ...
        self.username = os.getlogin() # <<< Added based on context from previous turns
    ...
    def _create_ui(self):
        ...
        # Row 2 (Timeline) removed
        self.frame.grid_rowconfigure(2, weight=1)  # Version history - flexible (Now row 2)
        self.frame.grid_rowconfigure(3, weight=0)  # Actions - fixed height (Now row 3)

        # Create content sections with card design
        self._create_header_section()
        self._create_metadata_section()
        # self._create_timeline_section() # <<< REMOVED
        self._create_version_history_section() # <<< Now at row 2
        self._create_action_section() # <<< Now at row 3
    ...
    # def _create_timeline_section(self): # <<< REMOVED
    #     pass

    # def _draw_timeline(self, event=None): # <<< REMOVED
    #     pass

    # def _show_timeline_tooltip(self, event): # <<< REMOVED
    #     pass
    ...
    # _show_timeline_tooltip removed
    ...
    def _filter_versions(self, event=None):
        ...
        for version_hash, info in self.versions_data:
            ...
            if filter_option == "Last 7 Days":
                # Check if version is within last 7 days
                try:
                    version_date = datetime.strptime(info["timestamp"], "%Y-%m-%d %H:%M:%S")
                    now = datetime.now()
                    if (now - version_date).days > 7:
                        continue
                except:
                    pass
    ...
    def _populate_version_tree(self, versions_data):
        """Populate tree with version data."""
        # Clear tree
        self.version_tree.delete(*self.version_tree.get_children())
        ...
        for i, (version_hash, info) in enumerate(sorted_versions):
        ...
        available_count = sum(1 for vh, _ in versions_data
                            if self._check_backup_exists(self.selected_file, vh))
        self.version_count_label.config(
            text=f"Showing {len(versions_data)} versions ({available_count} available)"
        )
                            ...
                            with file_monitor.lock:
                                ...
                                if os.path.exists(normalized_path):
                                    try:
                                        ...
                                        print(f"[{current_time}] [{self.username}] *** FORCED RESET of file monitoring after restore: {normalized_path} ***")
                                    except Exception as e:
                                        current_time = datetime.now(pytz.UTC).strftime("%Y-%m-%d %H:%M:%S")
                                        print(f"[{current_time}] [{self.username}] Error during forced reset: {str(e)}")
        ...
        if details:
            ...
            for key, value in details.items():
                ...
                value_label = tk.Label(
                    ...
                    font=("Segoe UI", int(11 * self.font_scale)),
                    anchor='w',
                    wraplength=int(350 * self.ui_scale)
                )
    ...
    def refresh_layout(self):
        ...
        # Redraw timeline if needed <<< REMOVED
