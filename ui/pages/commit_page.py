# ui/pages/commit_page.py - Enhanced with responsive design
...
class CommitPage:
    ...
    def __init__(self, parent, version_manager, backup_manager, settings_manager, shared_state, colors=None, ui_scale=1.0, font_scale=1.0):
        ...
        self.username = os.getlogin()  # Use system username
    ...
    def _update_suggestions(self):
        ...
        for i, suggestion in enumerate(self.suggested_messages[:4]):
            ...
            btn.pack(side='left', padx=(0 if i == 0 else self.SMALL_PADDING), pady=self.SMALL_PADDING)
    ...
    def _get_contextual_suggestions(self):
        ...
        # First add any relevant past messages
        if past_messages:
            suggestions.extend(past_messages[:2])
    ...
    def _get_past_commit_messages(self, file_path):
        """Get past commit messages for the file."""
        try:
            messages = []
            tracked_files = self.version_manager.load_tracked_files()
            normalized_path = os.path.normpath(file_path)
            ...
            if normalized_path in tracked_files:
                ...
                for version_info in versions.values():
                    msg = version_info.get("commit_message")
    ...
    def _suggest_messages(self, event=None):
        """Suggest messages based on current input."""
        current_text = self.commit_message_entry.get().lower()
        ...
        filtered = []
        for suggestion in self.suggested_messages:
            if current_text in suggestion.lower() and suggestion.lower() != current_text:
        ...
        if filtered:
            ...
            # Create buttons for filtered suggestions
            for i, suggestion in enumerate(filtered[:4]):
                btn = self._create_suggestion_button(suggestion)
                btn.pack(side='left', padx=(0 if i == 0 else self.SMALL_PADDING), pady=self.SMALL_PADDING)
    ...
    def _select_file(self):
        ...
        if file_path:
            ...
            # Schedule UI update after animation
            self.parent.after(600, enable_ui)
        else:
    ...
    def _animate_file_selected(self, file_path):
        ...
        # Animation content
        anim_frame = tk.Frame(overlay, bg=self.colors['primary'], padx=int(30 * self.ui_scale), pady=int(20 * self.ui_scale))
        anim_frame.pack(fill='both', expand=True)
        
        icon = tk.Label(
            anim_frame,
            text="📄",
            font=("Segoe UI", int(48 * self.font_scale)),
            fg=self.colors['white'],
            bg=self.colors['primary']
        )
        icon.pack(pady=(self.STANDARD_PADDING, 0))
        
        msg = tk.Label(
            anim_frame,
            text=f"File Selected",
            font=("Segoe UI", int(16 * self.font_scale), "bold"),
            fg=self.colors['white'],
            bg=self.colors['primary']
        )
        msg.pack()
        ...
        self.parent.after(500, close_animation)
    ...
    def _update_metadata_display(self):
        ...
        try:
            ...
            # Get current time in UTC with specific format
    ...
    def _commit_file_action(self, event=None):
        """Handle the commit action with animation and feedback."""
        if not self.selected_file or not os.path.exists(self.selected_file):
    ...
    def _continue_commit(self, commit_message, current_hash, last_hash, tracked_files):
        ...
        try:
            ...
            if is_first_commit and self.shared_state.file_monitor and hasattr(self.shared_state.file_monitor, 'add_new_file'):
                ...
                print(f"[{times['utc']}] [{self.username}] Started monitoring new file: {normalized_path}")
            ...
            # Hide progress and show success animation
            self._hide_progress_indicator()
            self._animate_commit_success()
    ...
    def _animate_commit_success(self):
        ...
        success_overlay.place(
            relx=0.5, rely=0.5,
            anchor='center',
            width=int(300 * self.ui_scale), height=int(180 * self.ui_scale)
        )
        ...
        check.pack(pady=(int(20 * self.ui_scale), int(10 * self.ui_scale)))
        ...
        self.parent.after(1000, success_overlay.destroy)
    ...
    def _on_file_updated(self, file_path):
        ...
        if file_path and os.path.exists(file_path):
            ...
            # Update suggestions based on file type
            self._update_suggestions()
        else:
            if self.shared_state.file_monitor:
                self.shared_state.file_monitor.set_file(None)
